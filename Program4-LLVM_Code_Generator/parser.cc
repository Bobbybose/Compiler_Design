...
static const char *const yytname[] =
{
  ...
  "'/'", "'%'", "'='", "'('", "')'", "'{'", "'}'", "';'", "INT", "FLOAT",
  "CHAR", "AUTO", "RETURN", "UMINUS", "$accept", "program",
  "statement_list", "statement", "$@1", "type", "expression", YY_NULLPTR
};
...
{
     -10,     2,    25,   -10,    -2,   -10,   -10,   -10,   -10,     0,
       0,   -10,   -10,   -10,   -10,   -10,     0,   -10,    13,    63,
     -10,   -10,    96,   -10,    74,    -3,     0,     0,     0,     0,
       0,     0,   -10,   -10,    47,   -10,     0,   -10,     1,     1,
     -10,   -10,   -10,   104,   -10,    85,   -10
};
...
{
       4,     0,     0,     1,     0,    27,    24,    25,    26,     0,
       0,     6,    12,    13,    14,    15,     0,     3,     0,     0,
      11,    22,     0,     4,     0,     0,     0,     0,     0,     0,
       0,     0,     5,    23,     0,    10,     0,     9,    16,    17,
      18,    19,    20,    21,     7,     0,     8
};
...
{
      21,    22,     3,     5,     6,     7,     8,    24,     9,    36,
      28,    29,    30,    10,    37,    20,    25,    38,    39,    40,
      41,    42,    43,     0,     0,    -2,     4,    45,     5,     6,
       7,     8,    34,     9,     0,     0,     0,     0,    10,     0,
      11,     0,     0,    12,    13,    14,    15,    16,     4,     0,
       5,     6,     7,     8,     0,     9,     0,     0,     0,     0,
      10,     0,    11,    44,     0,    12,    13,    14,    15,    16,
      26,    27,    28,    29,    30,    31,     0,     0,     0,     0,
      32,    26,    27,    28,    29,    30,    31,     0,     0,     0,
       0,    35,    26,    27,    28,    29,    30,    31,     0,     0,
       0,     0,    46,    26,    27,    28,    29,    30,    31,     0,
      33,    26,    27,    28,    29,    30,    31
};

static const yytype_int8 yycheck[] =
{
       9,    10,     0,     3,     4,     5,     6,    16,     8,    12,
       9,    10,    11,    13,    17,    17,     3,    26,    27,    28,
      29,    30,    31,    -1,    -1,     0,     1,    36,     3,     4,
       5,     6,    23,     8,    -1,    -1,    -1,    -1,    13,    -1,
      15,    -1,    -1,    18,    19,    20,    21,    22,     1,    -1,
       3,     4,     5,     6,    -1,     8,    -1,    -1,    -1,    -1,
      13,    -1,    15,    16,    -1,    18,    19,    20,    21,    22,
       7,     8,     9,    10,    11,    12,    -1,    -1,    -1,    -1,
      17,     7,     8,     9,    10,    11,    12,    -1,    -1,    -1,
      -1,    17,     7,     8,     9,    10,    11,    12,    -1,    -1,
      -1,    -1,    17,     7,     8,     9,    10,    11,    12,    -1,
      14,     7,     8,     9,    10,    11,    12
};
...
{
       0,    25,    26,     0,     1,     3,     4,     5,     6,     8,
      13,    15,    18,    19,    20,    21,    22,    27,    29,    30,
      17,    30,    30,    28,    30,     3,     7,     8,     9,    10,
      11,    12,    17,    14,    26,    17,    12,    17,    30,    30,
      30,    30,    30,    30,    16,    30,    17
};

/* YYR1[RULE-NUM] -- Symbol kind of the left-hand side of rule RULE-NUM.  */
static const yytype_int8 yyr1[] =
{
       0,    24,    25,    26,    26,    27,    28,    27,    27,    27,
      27,    27,    29,    29,    29,    29,    30,    30,    30,    30,
...
#line 228 "parser.y"
	...
	std::string converted_code = output_conversion(yyvsp[0].code);
...
#line 257 "parser.y"
	...
	std::string RHS_name = yyvsp[-1].addr->name();
	// Tracking RHS type
	Type RHS_type;
	// Tracking LHS type
	Type LHS_type;
	...
	// Obtaining RHS type
	if(dynamic_cast<Variable *>(yyvsp[-1].addr) != nullptr){	// If RHS is a variable
		RHS_type = symtab.get(RHS_name)->type;

		// Loading variable
		Address* temp = symtab.make_temp(yyvsp[-1].type);
		RHS_name = temp->name();
		Variable* var = symtab.make_variable(yyvsp[-1].addr->name());
		yyval.code += temp->name() + " = load " + get_type_string(RHS_type) + ", " + get_type_string(RHS_type) + " *" + var->location()->name() + "\n";
	}
	else
		RHS_type = yyvsp[-1].type;

	// Obtaining LHS type
	if(yyvsp[-4].type == Type::Auto)		// Variable being declared with auto as type
		LHS_type = RHS_type;
	else
		LHS_type = yyvsp[-4].type;
...
#line 353 "parser.y"
	...
	// Allocating space for variable
	Variable* var = symtab.make_variable(yyvsp[-1].code);	
...
#line 370 "parser.y"
	...
	std::string return_name = yyvsp[-1].addr->name();
	
	// Marking that a return statement is present
	return_statement = 1;
	
	// If returning a variable
	if(dynamic_cast<Variable *>(yyvsp[-1].addr) != nullptr){
		...
		Variable* var = symtab.make_variable(yyvsp[-1].addr->name());
		yyval.code += temp->name() + " = load " + get_type_string(return_type) + ", " + get_type_string(return_type) + " *" + var->location()->name() + "\n";
	}
	else
		return_type = yyvsp[-1].type;
	...
	if(return_type != Type::Int){
		if(return_type == Type::Float){
			...
			yyval.code += temp->name() + " = fptosi float " + return_name + " to i32\n";
			return_name = temp->name();
			...
			Address* temp = symtab.make_temp(Type::Int);
			yyval.code += temp->name() + " = sext i8 " + return_name + " to i32\n";
			return_name = temp->name();
			return_type = temp->type();
		}
	}
	...
	yyval.code += "ret " + get_type_string(return_type) + " " + return_name + "\n";
...
#line 438 "parser.y"
	...
	if(dynamic_cast<Variable *>(yyvsp[0].addr) != nullptr){
		...
		yyval.code += temp->name() + " = load " + exp_type + ", " + exp_type + " *" + var->location()->name() + "\n";
		
		// Initializing and setting new temp
		yyval.addr = symtab.make_temp(yyvsp[0].type);
		if(yyvsp[0].type == Type::Float)
			yyval.code += yyval.addr->name() + " = fneg float " + temp->name() + "\n";
		else
			yyval.code += yyval.addr->name() + " = sub " + exp_type + " 0, " + temp->name() + "\n";
	}
	// Else it is a constant
		...
		yyval.addr = symtab.make_temp(yyvsp[0].type);
		if(yyvsp[0].type == Type::Float)
			yyval.code += yyval.addr->name() + " = fneg float " + yyvsp[0].addr->name() + "\n";
		else
...
#line 510 "parser.y"
	...
	if(symtab.get(yyvsp[0].code) == nullptr){
		...
		Variable* var = symtab.make_variable(yyvsp[0].code);	
		
		yyval.code += var->location()->name() + " = alloca i32\n";
		yyval.addr = var;
	}
	else{
